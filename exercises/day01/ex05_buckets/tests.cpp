...
FillResult canFillRecursive(Bucket big_bucket, const vector<Bucket>& small_buckets, size_t index) {
  if(index >= small_buckets.size()) return {false, {}};
...
FillResult canFill(Bucket big_bucket, range auto&& small_buckets) {
  ...
  // trim irrelevant buckets
  small_buckets_sorted.erase(
    std::ranges::upper_bound(small_buckets_sorted, big_bucket),
    small_buckets_sorted.end()
  );  
  // skip zero small bucket if exists
  size_t start = 0;
  if(small_buckets_sorted.empty()) return {false, {}};
  if(small_buckets_sorted.front() == 0) start = 1;
  // call the recursive helper function, starting from index 0
  return canFillRecursive(big_bucket, small_buckets_sorted, start);
}
...
INSTANTIATE_TEST_SUITE_P(CanFillSimple,
                         ...
                         testing::Values(
                             ...
                             std::tuple{8, Buckets{3, 5}, true, FillPlan{{3, 1}, {5, 1}}},
...
INSTANTIATE_TEST_SUITE_P(CanFillIgnoreZeroSmallBuckets,
                         FillBucketTest,
                         testing::Values(
                             std::tuple{11, Buckets{3, 0, 2}, true, FillPlan{{3, 1}, {2, 4}}},
                             std::tuple{12, Buckets{3, 4, 0}, true, FillPlan{{3, 4}}},
...
INSTANTIATE_TEST_SUITE_P(CannotFillSimple,
                         ...
                         testing::Values(
                             ...
                             std::tuple{11, Buckets{5, 3}, false, FillPlan{}}
                            )
                        );
...
INSTANTIATE_TEST_SUITE_P(CanFillZeroBigBucket,
                         ...
                         testing::Values(
                             ...
                             std::tuple{0, Buckets{}, false, FillPlan{}}
                            )
                        );
