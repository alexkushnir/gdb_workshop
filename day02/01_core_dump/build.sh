#!/usr/bin/env bash
# day02/04_core_dump/build.sh — Builds all projects in this subdirectory
...
echo "  Building projects in $(basename "${SUBDIR}")..."

FAILED=()
BUILT=()

# Find all immediate subdirectories, sorted
mapfile -t PROJECTS < <(find "${SUBDIR}" -maxdepth 1 -mindepth 1 -type d | sort)
